...
**One engine, one number: claude-faf-mcp 7 scores all 33 slots with faf-cli 8's always-33 kernel — the same score faf-cli, faf-kernel and rust-faf-mcp give.** 513 tests.
...
- **The 12 enterprise slots stay in view.** faf-cli fills the 21 base slots and marks the 12 enterprise slots `slotignored` unless your app-type uses them; `faf_score` counts all 33.
...
- **Every empty slot says what fills it — "fact from repo".** faf_doctor, faf_score (details) and faf_go label each empty slot from faf_auto's own dry run: `fact from repo: <value>` when the repo holds it, `no fact in repo: answer it (faf_go), or fix project.type if it doesn't apply`, `yours: faf_go asks` for a 6W, or `the app-type leaves it out: faf_auto marks it slotignored`. faf_auto is named once, only when it has a fact to write. The dry run now reads project.faf exactly as faf_auto does, so a type faf fell back to (`# found: no classifying signals — fallback`) never makes faf_go send you back to a faf_auto with nothing to write.
...
### Fixed
- Fixed npm install hang that prompted for `npm fund` input
...
### Added
- **TYPE_DEFINITIONS Scoring Parity** - Ported from faf-cli v3.2.5
  - 94 project types + 38 aliases for accurate slot-based scoring
  - Single source of truth for project type detection
  - Ensures MCP scoring matches CLI scoring exactly

### Changed
- Scoring engine now uses TYPE_DEFINITIONS for slot applicability
- Improved type detection for CLI, fullstack, library, and other project types
