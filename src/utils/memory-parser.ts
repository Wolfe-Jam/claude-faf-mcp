 ...
 *
 * Ported from faf-cli for MCP context. Export only (no harvest/import).
 *
...
function generateFafSection(fafData: any): string {
  ...
  // Footer
  lines.push('---');
  lines.push(`*Seeded from project.faf by claude-faf-mcp tri-sync — ${new Date().toISOString().split('T')[0]}*`);
